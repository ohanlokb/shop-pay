...
export default function Product({ product, selected, setSelected }) {
  ...
  const updateQtyHandler = (type)=> {
    let newCart = checkout.checkoutItems.map((p)=>{
      if (p._uid == product._uid) {
        return {
          ...p,
          qty: type == '+' ? product.qty + 1 : product.qty - 1,
        }
      }
      return p;
    });
  ...
  const removeProduct = (uid) => {
    let newCart = checkout.checkoutItems.filter((p)=>{
      return p._uid != uid;
    });
    dispatch(updateCart(newCart));
  };
        ...
        <div className={styles.col}>
          ...
          <div className={styles.product__priceQty}>
            <div className={styles.product__priceQty_price}>
              <span className={styles.price}>${(product.price * product.qty).toFixed(2)}</span>
              {
                product.price !== product.priceBefore && (
                  <span className={styles.priceBefore}>${product.priceBefore.toFixed(2)}</span>
                )
              }
              {
                product.discount>0 && (
            ...
            <button disabled={product.qty < 2} onClick={()=>updateQtyHandler('-')}>-</button>
            <span>{product.qty}</span>
            <button disabled={product.qty == product.quantity} onClick={()=>updateQtyHandler('+')}>+</button>
            </div>
          </div>
          <div className={styles.product__shipping}>
            {
              product.shipping ? `+$${product.shipping.toFixed(2)} Shipping fee`: 'Free Shipping'
            }
          </div>
          {
            product.quantity < 1 && (
              <div className={styles.notAvailable}>
                Currently out of stock, add it to your wishlist as it may get restocked.
              </div>
            )
          }
        </div>
